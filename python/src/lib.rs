...
#[inline(always)]
fn py_err(from: pure_magic::Error) -> PyErr {
    match &from {
        pure_magic::Error::Io(e) => PyIOError::new_err(e.to_string()),
        _ => PyValueError::new_err(from.to_string()),
    }
}
...
impl MagicDb {
    ...
    pub fn new() -> PyResult<Self> {
        magic_db::load().map(Self).map_err(py_err)
    }
